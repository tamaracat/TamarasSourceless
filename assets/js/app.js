...
generateRandomNumberArticle: function(){

  
  var randomSource = Math.floor(Math.random() * 5);
...
DisplayArticle: function(response){   // Variable to store number of results
    ...
    int = API_Call.generateRandomNumberArticle();

    console.log("DisplayArticle" + response.articles);

    var status = response.status;

    var articleTitle = results[int].title;

           
    // Grab description from API
    var descriptionTitle = results[int].description;
    // console.log(descriptionTitle);
            
    var image = results[int].urlToImage;
   
    var articleURL = results[int].url;
